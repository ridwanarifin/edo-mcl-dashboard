  ...
  <div class="pa-5">
    ...
    <v-dialog v-model="modalDeleteDialog" :disabled="loadingDelete" max-width="600px" persistent>
      ...
      >
        ...
        <v-card-actions>
          <v-btn :disabled="loadingDelete" :loading="loadingDelete" text color="error" @click.stop="modalDeleteDialog = false">
            Cancel
          </v-btn>
          <v-spacer />
          <v-btn :disabled="loadingDelete" :loading="loadingDelete" dark color="error" @click="handle_delete_edo">
            Yes, Delete
          </v-btn>
        </v-card-actions>
      </v-alert>
...
<script>
import {
  getColorStatus, isCanReject, isCanSendToConsignee,
  isAdminCanEdit,
  isAdminCanDelete,
  isAdminCanPrint
} from '@/utils'
